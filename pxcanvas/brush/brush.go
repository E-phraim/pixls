...
func Cursor(config apptype.PxCanvasConfig, brush apptype.BrushType, v *desktop.MouseEvent, x int, y int) []fyne.CanvasObject {
...
func TryBrush(appState *apptype.State, canvas apptype.Brushable, v *desktop.MouseEvent) bool {
	switch {
	case appState.BrushType == Pixel:
		return TryPaintPixel(appState, canvas, v)
	default:
		return false
	}
}

func TryPaintPixel(appState *apptype.State, canvas apptype.Brushable, v *desktop.MouseEvent) bool {
	x, y := canvas.MouseToCanvasXY(v)
	if x != nil && y != nil && v.Button == desktop.MouseButtonPrimary {
