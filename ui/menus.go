...
import (
	...
	"github.com/E-phraim/pixls/util"
)
		...
		dialog.ShowForm("New Image", "Create", "Cancel", formItems, func(ok bool) {
			if ok {
				...
				if widthEntry.Validate() != nil {
					dialog.ShowError(errors.New("invalid width"), app.PixlsWindow)
				} else {
					pixelWidth, _ = strconv.Atoi(widthEntry.Text)
				}
				if heightEntry.Validate() != nil {
					dialog.ShowError(errors.New("invalid height"), app.PixlsWindow)
				} else {
					pixelWidth, _ = strconv.Atoi(heightEntry.Text)
				}
				app.PixlsCanvas.NewDrawing(pixelWidth, pixelHeight)
			}
		}, app.PixlsWindow)
...
func SetupMenus(app *AppInit) {
	...
	app.PixlsWindow.SetMainMenu(mainMenu)
}
...
func saveFileDialog(app *AppInit) {
	dialog.ShowFileSave(func(uri fyne.URIWriteCloser, e error) {
		if uri == nil {
			...
			err := png.Encode(uri, app.PixlsCanvas.PixelData)
			if err != nil {
				dialog.ShowError(err, app.PixlsWindow)
				return
			}
...
func BuildSaveMenu(app *AppInit) *fyne.MenuItem {
	return fyne.NewMenuItem("Save", func() {
		if app.State.FilePath == "" {
			...
			err = png.Encode(fh, app.PixlsCanvas.PixelData)
			if err != nil {
				dialog.ShowError(err, app.PixlsWindow)
				return
			}
	...
	return fyne.NewMenuItem("Open...", func ()  {
		dialog.ShowFileOpen(func (uri fyne.URIReadCloser, e error)  {
			if uri == nil {
				...
				app.PixlsCanvas.LoadImage(image)
				app.State.SetFilePath(uri.URI().Path())
